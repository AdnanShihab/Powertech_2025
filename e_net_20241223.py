...
class power_system:
    ...
    def power_flow_2025_jan(self):
        ...
        pp.create_sgen(self.net, bus=12, p_mw=self.x_gen_bus_12_mw, name="CHP_ind bus12")
        pp.create_sgen(self.net, bus=1, p_mw=self.x_gen_bus_1_mw, name="CHP_ind bus1")
    ...
    def power_flow_2025_jan_new(self):
        ...
        # ----------------- CHP -----------------
        x_chp_bus = self.chp_bus
        pp.create_gen(self.net, bus=x_chp_bus, p_mw=0, name="CHP")       # index 0
        pp.create_gen(self.net, bus=12, p_mw=0, name="CHP_ind bus12")   # index 1
        pp.create_gen(self.net, bus=1, p_mw=0, name="CHP_ind bus1")  # index 2
        ...
        for i, ((load_idx, load_row), (irradiance_idx, irradiance_row)) in \
            ...
            # *************************** CHP ***************************
            self.net.gen.at[0, 'p_mw'] = self.chp_p_mw
            self.net.gen.at[1, 'p_mw'] = self.x_gen_bus_12_mw
            self.net.gen.at[2, 'p_mw'] = self.x_gen_bus_1_mw
            ...
            result = {
                ...
                'demand_mw': self.net.res_load.p_mw.sum(),  # summing the total load at each hour (t)
                # 'line_loss_mw': self.net.res_line.pl_mw.values[0],
                'chp_mw': self.net.res_gen.p_mw.sum(),
                'pv_wt_mw': self.net.res_sgen.p_mw.sum(),  # summing the total PV gen at each hour (t)
                'bess_mw': self.net.res_storage.p_mw.sum(),
                'ext_grid_mw': self.net.res_ext_grid.p_mw.values[0],
                # 'bess_mwh': res_bess_mwh
            }
    ...
    def power_flow_2025_jul(self):
        ...
        x_chp_bus = self.chp_bus
        pp.create_sgen(self.net, bus=x_chp_bus, p_mw=0, q_mvar=0, name="CHP")
        pp.create_sgen(self.net, bus=12, p_mw=self.x_gen_bus_12_mw, name="CHP_ind bus12")  # Peak load = 29.3 MW
        pp.create_sgen(self.net, bus=1, p_mw=self.x_gen_bus_1_mw, name="CHP_ind bus1")  # Peak load = 20.8 MW
